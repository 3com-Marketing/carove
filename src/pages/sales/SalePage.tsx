...
import { Loader2, ArrowLeft, PlusCircle, Banknote, CreditCard, ArrowRightLeft, Landmark, Lock, AlertTriangle } from 'lucide-react';
import { Alert, AlertDescription } from '@/components/ui/alert';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip';
import { getBuyers, getProfiles, getVehicleById, getReservationById, getPaymentsByReservation, createSale, createFinanceSimulation, convertReservationToSale } from '@/lib/supabase-api';
...
export default function SalePage() {
  ...
  const reservationPaymentMethod = reservationPayments.length > 0 ? reservationPayments[0].payment_method : 'efectivo';
  const reservationDate = reservationPayments.length > 0 ? reservationPayments[0].payment_date : reservation?.created_at;
  ...
  const isFullyCovered = Math.abs(pendingAmount) < 0.01;
  const canSubmit = buyerId && price > 0 && sellerId && hasAnyPayment && isFullyCovered && !submitting && (!hasFinancedBlock || financeSelection);
  

  const handleConfirm = async () => {
    if (!canSubmit || !user || !vehicle) return;
    setSubmitting(true);
    setError('');
    try {
      ...
      // Convert reservation if applicable
      if (reservationId) {
        try {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="text-xl font-bold tracking-tight">Cerrar Venta</h1>
          <p className="text-sm text-muted-foreground">
            {vehicle.brand} {vehicle.model} · {vehicle.plate} · {vehicle.version}
          </p>
        </div>
      </div>
      ...
      {hasAnyPayment && !isFullyCovered && pendingAmount > 0.01 && (
        <Alert className="border-yellow-500/50 bg-yellow-500/10">
          <AlertTriangle className="h-4 w-4 text-yellow-600" />
          <AlertDescription className="text-yellow-700">
            <strong>Pago pendiente: {formatCurrency(pendingAmount)}</strong>. Registra el pago completo en el plan de pagos para poder confirmar la venta.
          </AlertDescription>
        </Alert>
      )}
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <span>
                <Button onClick={handleConfirm} disabled={!canSubmit} size="lg">
                  {submitting && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
                  Confirmar venta
                </Button>
              </span>
            </TooltipTrigger>
            {!isFullyCovered && pendingAmount > 0.01 && (
              <TooltipContent>
                <p>Falta registrar {formatCurrency(pendingAmount)} para completar la venta</p>
              </TooltipContent>
            )}
          </Tooltip>
        </TooltipProvider>
